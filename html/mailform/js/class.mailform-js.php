...
(function( $ ) {
	...
	var mailform_dt    = $( 'form#mail_form div' );
...
EOM;
	...
	function error_span( e, dt, comment, bool ) {
		if ( bool === true ) {
			var m = e.parents( 'div' ).find( 'span.error_blank' ).text( dt + 'が' + comment + 'されていません' );
		} else {
			var m = e.parents( 'div' ).find( 'span.error_blank' ).text( '' );
		}
	}
...
EOM;
	...
	function required_check() {
		...
		for ( var i = 0; i < mailform_dt.length; i++ ) {
			// required input
			if ( mailform_dt.eq(i).find( 'input' ).length && mailform_dt.eq(i).hasClass( 'required' ) ) {

				var elements = mailform_dt.eq(i).find( 'input' );
